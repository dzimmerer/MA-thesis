...
\begin{figure}[h!]
	...
	\caption{Misclassifications of the spiking DBNs. Often $9$ and $4$ are mixed up. The first three are classified as a $9$, the third one is classified as a $4$. The images all share a lot of features with the suggested class and the correct class was always the second most probable.}
	\label{fig:stripes}
\end{figure}
...
Another factor is the simulated runtime for each sample. It is quite apparent from table x (Table: CNN vs CUBA , simulated time , performance, run time  ) that a longer classification time benefits the performance. But due to it's more deterministic nature the CNN approach shows less variations as the simulation time changes. 
...
The resulting weights are visualized in Fig. X
...
To evaluate the extracted features, we show a reconstruction and completion of the input data (see Fig X and Y) and the classification performance. 
The trained DBN reaches a peak performance of $90 \%$ on the training set (see Fig X).
